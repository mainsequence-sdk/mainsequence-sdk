...
- Removed Agent-scoped one-shot `respond()`, `stream_response()`, and Agent-level runtime-access
  helpers. Agent work uses an `AgentSession` and its chat or A2A methods; direct model inference is
  not a TAU Agent client operation.
...
- Added the backend-owned CodeRepositoryBranch provisioning lifecycle
  projections to strict Git-context, list, and detail responses.
...
- Added canonical, JSON-compatible serialization for `datetime.timedelta`
  values in TimeIndexTableUpdater configuration hashing and reconstruction.
...
- Made name-based Secret reads preserve metadata-only list responses while
  hydrating the unique match through the value-bearing UID detail endpoint.
