...
from ..data_nodes.constants import (
...
def backfill_portfolio_metadata_from_legacy_portfolios(
    portfolios: Iterable[Any],
    *,
    updater: Any | None = None,
) -> list[PortfolioMetadata]:
    """Copy legacy PortfolioAbout descriptions into the canonical metadata table."""
    resolved_updater = updater or default_portfolio_metadata_updater()
    upserted: list[PortfolioMetadata] = []
    for portfolio in portfolios:
        unique_identifier = _extract_portfolio_index_asset_unique_identifier(portfolio)
        description = _extract_legacy_portfolio_about_description(portfolio)
        if unique_identifier is None or description is None:
            continue
        upserted.append(
...
def _extract_portfolio_index_asset_unique_identifier(value: Any) -> str | None:
    if value is None:
        return None
    if isinstance(value, dict):
        for key in ("portfolio_index_asset", "index_asset", "asset"):
            unique_identifier = _extract_unique_identifier(value.get(key))
            if unique_identifier is not None:
                return unique_identifier
        return _extract_unique_identifier(value)

    for attr_name in ("portfolio_index_asset", "index_asset", "asset"):
        unique_identifier = _extract_unique_identifier(getattr(value, attr_name, None))
        if unique_identifier is not None:
            return unique_identifier
    return _extract_unique_identifier(value)


def _extract_legacy_portfolio_about_description(value: Any) -> str | None:
    if value is None:
        return None
    if isinstance(value, dict):
        for key in ("target_portfolio_about", "portfolio_about"):
            description = _extract_portfolio_description(value.get(key))
            if description is not None:
                return description
        return None

    for attr_name in ("target_portfolio_about", "portfolio_about"):
        description = _extract_portfolio_description(getattr(value, attr_name, None))
        if description is not None:
            return description
    return None
