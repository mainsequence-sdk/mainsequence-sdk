...
7. `.agents/skills/mainsequence/command_center/workspaces/widgets/app_components/SKILL.md`

Also load:

8. `.agents/skills/mainsequence/command_center/api_contracts/resource_api/SKILL.md` when the API exposes a canonical collection, collection controls, entity summary, selection action, or action preflight
9. `.agents/skills/mainsequence/command_center/workspaces/builder/SKILL.md` when the API is tied to mounted widgets, workspace payloads, or workspace mutation
10. `.agents/skills/mainsequence/command_center/workspaces/widgets/tables/SKILL.md` when the API returns
   `core.tabular_frame@v1`, `meta.tableVisuals`, or table/pro-table widget-facing data
11. `.agents/skills/mainsequence/command_center/workspaces/widgets/tabular_transform/SKILL.md` when the API
    response will be reshaped by `core__tabular-transform`
12. `.agents/skills/mainsequence/command_center/api_contracts/adapter_from_api/SKILL.md` when the API must be consumed through a Command Center Adapter from API connection
13. `.agents/skills/mainsequence/command_center/workspaces/api_mock_prototyping/SKILL.md` when the contract should be validated in AppComponent mock mode before backend deployment
...
- load `.agents/skills/mainsequence/command_center/api_contracts/resource_api/SKILL.md` when the route is a resource collection, summary, bulk action, or preflight
- load `.agents/skills/mainsequence/command_center/workspaces/widgets/app_components/SKILL.md`
- also load `.agents/skills/mainsequence/command_center/workspaces/builder/SKILL.md` when the API is coupled to mounted workspace widgets or workspace payloads
- load `.agents/skills/mainsequence/command_center/api_contracts/adapter_from_api/SKILL.md` when the API will be consumed by connection-first workspace dataflow
