...
how to verify platform behavior, and how to use the project-state files kept under `astro/`.

Before any non-trivial Main Sequence work, verify that this `AGENTS.md` matches the latest
version at
`file:/Users/jose/code/MainSequenceClientSide/mainsequence-sdk/docs/astro/AGENTS.md`
...
- when work is delegated or queued for later, write the task in `astro/tasks.md` according to the skill that should execute it
- each delegated or queued task in `astro/tasks.md` must state:
...
- project scaffolding, folder structure, and standard repository layout:
  `docs/astro/project_builder/SKILL.md`
- project-state reconciliation, milestone logging, blocker recording, and next-step updates under `astro/`:
...
1. `docs/astro/project_builder/SKILL.md`
2. the relevant domain skill
3. `docs/astro/maintenance/local_journal/SKILL.md` after material work if verified state, blockers, scope, next actions, stable references, or historical notes changed
...
Always use `docs/astro/project_builder/SKILL.md` as the source of truth for project scaffolding, folder structure, and standard repository layout.
...
## Project-State Files Under `astro/`

The repository keeps project-state files under `astro/`:
...
- `docs/astro/maintenance/local_journal/SKILL.md` for:
  - `astro/brief.md`
  - `astro/tasks.md`
  - `astro/record.md`
  - `astro/status.md`
