...
- `mainsequence project update AGENTS.md` is project-scoped. It resolves the target project first, then creates the current Main Sequence `AGENTS.md` bootstrap when the file is missing. If an existing `AGENTS.md` has no Main Sequence managed marker, the command treats it as legacy/unmanaged and replaces the whole file. If the managed marker exists, the command updates only that managed block.
