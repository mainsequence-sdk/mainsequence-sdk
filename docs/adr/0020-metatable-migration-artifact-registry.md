...
0031, the shown `code_repository_context` is inserted internally from the
frozen Git-native context for local requests. Authenticated deployed runtimes
omit this transport field because the backend derives the same context from
the verified runtime target. Application callers never provide it:
...
exists, otherwise the process-frozen CodeRepositoryBranch MetaTables data
source.
...
`ensure_alembic_registry()` is a remote resolve-or-create operation, not an
unconditional collection create and not a class-cache lookup. It first resolves
the exact DataSource/schema/table physical identity, validates that any match is
the expected platform-managed, Alembic-managed provider root, and reuses it.
Only a missing registry is collection-created.

The reservation request generated by the SDK contains the resolved data source
and provider identity. The JSON below is the local-checkout wire projection;
the SDK inserts `code_repository_context` internally, while an authenticated
deployed runtime omits it and relies on backend-derived runtime context:
