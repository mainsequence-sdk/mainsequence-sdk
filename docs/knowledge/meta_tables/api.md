...
| `project_context` | Internal wire-only consistency context for a genuine local checkout. It is not accepted as caller input by the SDK. The SDK derives the exact persisted `project_branch_uid` from local Project/Git state; deployed branch-owned runtimes omit the field and the backend derives branch and environment scope from authentication. |
