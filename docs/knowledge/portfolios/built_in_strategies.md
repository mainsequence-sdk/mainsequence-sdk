...
Portfolios ships with a small set of built-in signals, rebalancers, and price helpers.
...
Signal strategies usually inherit `SignalWeights`.
...
This is the portfolio asset-signal shape. It does not restrict generic DataNodes,
...
This is one of the cleanest examples of how Artifacts and Portfolios work together.
...
Portfolios also ships with price-side helpers, because the portfolio engine needs prices in a stable format.

### `InterpolatedPrices`

Use this when:

- you want Portfolios to fetch and normalize price bars from an upstream source
...
- produces the price table shape Portfolios expects

This is the normal price node inside a portfolio workflow pipeline.
...
- maps the file into the long-form asset price schema Portfolios expects
...
Intraday strategies are more sensitive to calendars, bar quality, and interpolation settings. They are not the best first Portfolios project unless you actually need them.
