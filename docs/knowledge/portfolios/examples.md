...
The fastest way to understand Portfolios is to see a few concrete patterns.
...
- you want the smallest realistic portfolio workflow
...
```python
from mainsequence.markets.portfolios.contrib.signals import MarketCap
...
portfolio_config = PortfolioConfiguration(
    ...
    portfolio_markets_configuration=PortfolioMarketsConfig(
        portfolio_name="Demo Portfolio - Direct Config"
    ),
)
...
- it shows the normal Portfolios shape without extra indirection
...
- it is one of the cleanest smoke tests for Portfolios
...
- you want to ingest a portfolio instead of building it inside Portfolios
...
- how Portfolios consumes Artifact-backed operational inputs
...
- it shows that Portfolios can host more sophisticated signal-generation logic than fixed weights or simple filters
...
- it shows how much more sensitive intraday Portfolios workflows are to calendar and price-quality assumptions
