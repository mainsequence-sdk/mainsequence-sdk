...
Price handling is one of the most misunderstood parts of Portfolios.

This page explains how prices move through the portfolio engine and why "forward fill" in Portfolios actually means three different things.

## The two-stage price process

Portfolios does not go straight from raw bars to portfolio returns.
...
At the end of this stage, Portfolios has a clean long-form price table.
...
`InterpolatedPrices` is the main price node in the Portfolios pipeline.
...
Portfolios has three different fill systems:
...
## Why Portfolios subtracts `TIMEDELTA`
...
That small subtraction exists to avoid boundary problems when Portfolios checks whether a weight is still valid.
...
When a portfolio workflow looks wrong, check these in order.
