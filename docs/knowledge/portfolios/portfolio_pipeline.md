...
This page explains how a portfolio workflow is built from end to end.

The single most useful way to think about Portfolios is this:
...
- a `SignalWeights` instance provides signal weights
...
If `assets_category_unique_id` is present, Portfolios resolves the portfolio universe from that asset category.
...
This tells Portfolios how to get prices into a shape the portfolio engine can use.
...
This is a defining Portfolios design choice.

Portfolios uses **direct instance injection**, not string-based runtime lookups.
...
Portfolios applies fee drag based on turnover:
...
Usually inherits `SignalWeights`.
...
This is the portfolio signal contract. It is the standard asset-table specialization
...
Portfolios builds a derived timeline.

### Daily portfolios

When `upsample_frequency_id` is daily:

- Portfolios uses the rebalance strategy calendar
...
- Portfolios uses a `pd.date_range(...)` style timeline
...
This is the smallest realistic Portfolios pattern worth remembering:

```python
from mainsequence.markets.portfolios.contrib.signals.fixed_weights import AUIDWeight, FixedWeights
