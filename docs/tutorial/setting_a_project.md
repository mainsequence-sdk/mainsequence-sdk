...
Key points:
- `.env` is generated by the CLI during local setup and includes project runtime variables (for example `MAINSEQUENCE_ACCESS_TOKEN`, `MAINSEQUENCE_REFRESH_TOKEN`, `MAINSEQUENCE_ENDPOINT`, and `MAIN_SEQUENCE_PROJECT_UID`). Local checkouts resolve their active ProjectBranch from Git and do not persist a branch UID. Platform-deployed runtimes instead receive an authoritative ProjectBranch UID and Organization Environment UID from the backend because built images are not Git checkouts.
